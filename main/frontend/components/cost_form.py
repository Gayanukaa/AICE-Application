...
def show_cost_form(user_id: str):
    ...
    with col3:
        applicant_type = st.selectbox("Applicant Type", ["Select Applicant type", "Domestic", "International"])
    with col4:
    ...
    if st.button("Start Cost Breakdown"):
        if not university or not course or not location or applicant_type == "Select Applicant type":
